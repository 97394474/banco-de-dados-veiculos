...
def listar_veiculos():
    conexao = conectar()
    meu_cursor = conexao.cursor()
    sql = 'SELECT v.id, v.id_modelo, m.nome AS nome_modelo, v.ano, v.placa FROM veiculos v JOIN modelos m ON v.id_modelo = m.id'
    meu_cursor.execute(sql)
    linhas = meu_cursor.fetchall()
    print(f'{"id":<5}{"id_modelo":<6}{"modelo":^10}{"ano":<10}{"placa":<0}')
